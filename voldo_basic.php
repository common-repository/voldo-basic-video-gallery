...
function voldo_basic_head() {
	
	$site_url = get_option( 'siteurl' );
			
			
}
function voldo_basic($content){
	$content = preg_replace_callback("/\[voldo_basic ([^]]*)\/\]/i", "voldo_basic_render", $content);
	return $content;
	
}

function voldo_basic_render($tag_string){
$contador=rand(9, 9999999);
	$site_url = get_option( 'siteurl' );
global $wpdb; 	
$table_name = $wpdb->prefix . "voldo_basic";	
	...
	if($auxivideo!="") {
		...
		if(strstr($auxivideo, "http")) {
			if(strpos($auxivideo, "youtube")>0) {
				$auxivideo=getYTidvoldo($auxivideo);
				$auxtipo=2;
...
$texto.='&cantidad='.$cont1;
	...
	$output='
	<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'.$width.'" height="'.$height.'" id="voldo'.$id.'-'.$contador.'" title="'.$tags.'">
  <param name="movie" value="'.$site_url.'/wp-content/plugins/voldo-basic-video-gallery/voldo_basic.swf" />
  ...
  <param name="expressinstall" value="'.$site_url.'/wp-content/plugins/voldo-basic-video-gallery/Scripts/expressInstall.swf" />
  <!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
  <!--[if !IE]>-->
  <object type="application/x-shockwave-flash" data="'.$site_url.'/wp-content/plugins/voldo-basic-video-gallery/voldo_basic.swf" width="'.$width.'" height="'.$height.'">
    <!--<![endif]-->
    <param name="quality" value="high" />
    <param name="wmode" value="transparent" />
    	<param name="flashvars" value="'.$texto.'" />
...
<!--
swfobject.registerObject("voldo'.$id.'-'.$contador.'");
//-->
</script><br/>'.$ligtext;
	return $output;
}
...
function widget_voldo_basic($args) {

 
  
    extract($args);
	
	  $options = get_option("widget_voldo_basic");
  if (!is_array( $options ))
{
$options = array(
      'title' => 'voldo Basic',
...
$options = array(
  ...
  if ($_POST['voldo-Submit'])
  {
    $options['title'] = htmlspecialchars($_POST['voldo-WidgetTitle']);
	 $options['id'] = htmlspecialchars($_POST['voldo-WidgetId']);
    update_option("widget_voldo_basic", $options);
  }
...
if(empty($myrows)) {
	...
	echo '
	<p>First create a new gallery of videos, from the administration of BASIC voldo plugin.</p>
	';
}

else {
	$contaa1=0;
	$selector='<select name="voldo-WidgetId" id="voldo-WidgetId">';
...
echo '
  <p>
    <label for="voldo-WidgetTitle">Widget Title: </label>
    <input type="text" id="voldo-WidgetTitle" name="voldo-WidgetTitle" value="'.$options['title'].'" /><br/>
	<label for="voldo-WidgetTitle">voldo Video Gallery ID: </label>
   '.$selector.'
    <input type="hidden" id="voldo-Submit" name="voldo-Submit" value="1" />
  </p>
';
}
...
function voldo_basic_init(){
	register_sidebar_widget(__('voldo Basic'), 'widget_voldo_basic');
	register_widget_control(   'voldo Basic', 'voldo_basic_control', 300, 300 );
}

function voldo_basic_add_menu(){	
	if (function_exists('add_options_page')) {
		
		add_menu_page('voldo_basic', 'voldo Basic', 8, basename(__FILE__), 'voldo_basic_panel');
	}
}
if (function_exists('add_action')) {
	add_action('admin_menu', 'voldo_basic_add_menu'); 
}
add_action('wp_head', 'voldo_basic_head');
add_filter('the_content', 'voldo_basic');
add_action('activate_voldo_basic/voldo_basic.php','voldo_basic_instala');
add_action('deactivate_voldo_basic/voldo_basic.php', 'voldo_basic_desinstala');
add_action("plugins_loaded", "voldo_basic_init");
?>